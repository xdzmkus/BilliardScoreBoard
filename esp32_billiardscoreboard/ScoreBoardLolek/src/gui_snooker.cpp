...
static uint16_t ply1Scores = 0;
static uint16_t ply2Scores = 0;
...
static void ui_calculateScores()
{
    ...
    ply1Scores = _handicapP1;
    ply2Scores = _handicapP2;
    ...
    if (_isControl && ballsTable.red >= 0 && ballsTable.yellow >= 0 && ballsTable.green >= 0 && ballsTable.brown >= 0 && ballsTable.blue >= 0 && ballsTable.pink >= 0 && ballsTable.black >= 0)
    {
        ...
        if (ply1Scores + scoresOnTable < ply2Scores) lv_obj_add_state(ui_NPlyLabelScore1, LV_STATE_USER_1);
        else lv_obj_clear_state(ui_NPlyLabelScore1, LV_STATE_USER_1);
        if (ply2Scores + scoresOnTable < ply1Scores) lv_obj_add_state(ui_NPlyLabelScore2, LV_STATE_USER_1);
...
static void ui_event_ScreenSnooker(lv_event_t* e)
{
    ...
    if (event_code == LV_EVENT_SCREEN_LOADED)
    {
        ...
        _handicapP1 = strtoul(lv_textarea_get_text(ui_STextAreaHandicap1), &output, 10);
        _handicapP2 = strtoul(lv_textarea_get_text(ui_STextAreaHandicap2), &output, 10);
