...
uint16_t snookerPly1Scores = 0;
uint16_t snookerPly2Scores = 0;
...
static bool isAudioDisabled = false;
...
String gui_getState()
{
    ...
    if (numberActions > 0)
    {
        ...
        msg += snookerPly1Scores;  // idx = 15
        msg += delimiter;

        msg += snookerPly2Scores;  // idx = 16
        msg += delimiter;

        for (uint8_t idx = 0; idx < numberActions; idx++)
        {
            msg += historyActions[idx]; // idx = 17+
            msg += delimiter;
        }
    }
...
void gui_restoreState(String& value, uint8_t idx)
{
    ...
    case 15:
    case 16:
        break;  // do not restore snooker scores - it's calculated
    default:
