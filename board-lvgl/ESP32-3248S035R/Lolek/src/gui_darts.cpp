...
static lv_obj_t* ui_DPanelPlayers;
static lv_obj_t* ui_DPanelPly1;
static lv_obj_t* ui_DLabelPly1;
static lv_obj_t* ui_DPanelPly2;
static lv_obj_t* ui_DLabelPly2;
static lv_obj_t* ui_DPanelPly3;
static lv_obj_t* ui_DLabelPly3;
static lv_obj_t* ui_DPanelGame;
static lv_obj_t* ui_DLabelSuggest;
static lv_obj_t* ui_DLabelScore;
static lv_obj_t* ui_DPanelHistory;
static lv_obj_t* ui_DLabelHistory;
static lv_obj_t* ui_DButtonSector;
static lv_obj_t* ui_DLabelSector;
static lv_obj_t* ui_DButtonDouble;
static lv_obj_t* ui_DLabelDouble;
static lv_obj_t* ui_DButtonTriple;
static lv_obj_t* ui_DLabelTriple;
static lv_obj_t* ui_DLabelHome;
static lv_obj_t* ui_DLabelReset;
static lv_obj_t* ui_DLabelTrash;
static lv_obj_t* ui_DPanelReset;
static lv_obj_t* ui_DLabelNew;
static lv_obj_t* ui_DButtonYes;
static lv_obj_t* ui_DLabelYes;
static lv_obj_t* ui_DButtonNo;
static lv_obj_t* ui_DLabelNo;
static lv_obj_t* ui_DPanelRemove;
static lv_obj_t* ui_DLabelRemove;
static lv_obj_t* ui_DButtonRemoveYes;
static lv_obj_t* ui_DLabelRemoveYes;
static lv_obj_t* ui_DButtonRemoveNo;
static lv_obj_t* ui_DLabelRemoveNo;
static lv_obj_t* ui_DPanelKB;
static lv_obj_t* ui_DTextArea;
static lv_obj_t* ui_DKeyboard;
...
static void ui_calculate_score()
{
    ...
    for (auto it = dartsToShow->cbegin(); it != dartsToShow->cend(); ++it)
    {
        ...
        if (it->getType() == OVERALL)
        {
            scores = it->getScore(); // replace scores
        }
        else
        {
            scores += it->getScore(); // add value
        }
    }
...
static void ui_event_updatePly1Name(lv_obj_t* obj)
{
	...
	gui_setPlayerName(DARTS_PLAYER1, newName);
	lv_label_set_text(ui_DLabelPly1, newName);
}
static void ui_event_updatePly2Name(lv_obj_t* obj)
{
	const char* newName = lv_textarea_get_text(obj);
	gui_setPlayerName(DARTS_PLAYER2, newName);
	lv_label_set_text(ui_DLabelPly2, newName);
}
static void ui_event_updatePly3Name(lv_obj_t* obj)
{
	const char* newName = lv_textarea_get_text(obj);
	gui_setPlayerName(DARTS_PLAYER3, newName);
	lv_label_set_text(ui_DLabelPly3, newName);
}
...
static void ui_event_DPanelPly1(lv_event_t* e)
{
    ...
    else if(event_code == LV_EVENT_LONG_PRESSED)
	{
    	...
    	lv_textarea_set_text(ui_comp_get_child(ui_NameKeyboard, UI_COMP_NAMEKEYBOARD_NAMETEXTAREA), lv_label_get_text(ui_DLabelPly1));
...
static void ui_event_DPanelPly2(lv_event_t* e)
{
    ...
    else if(event_code == LV_EVENT_LONG_PRESSED)
	{
    	...
    	lv_textarea_set_text(ui_comp_get_child(ui_NameKeyboard, UI_COMP_NAMEKEYBOARD_NAMETEXTAREA), lv_label_get_text(ui_DLabelPly2));
        _ui_flag_modify(ui_NameKeyboard, LV_OBJ_FLAG_HIDDEN, _UI_MODIFY_FLAG_REMOVE);
    }
}
static void ui_event_DPanelPly3(lv_event_t* e)
{
	...
	{
		ui_comp_set_callback(ui_NameKeyboard, ui_event_updatePly3Name);
    	lv_textarea_set_text(ui_comp_get_child(ui_NameKeyboard, UI_COMP_NAMEKEYBOARD_NAMETEXTAREA), lv_label_get_text(ui_DLabelPly3));
        _ui_flag_modify(ui_NameKeyboard, LV_OBJ_FLAG_HIDDEN, _UI_MODIFY_FLAG_REMOVE);
    }
}
...
static void ui_event_DTextArea(lv_event_t* e)
{
    ...
    if (event_code == LV_EVENT_READY)
    {
		...
		const char* text = lv_textarea_get_text(ui_DTextArea);
        ...
        if(dartToAdd == OVERALL && isAssignment == false)
        {
            dartToAdd = SCORES;
        }
        if(isAssignment == true && dartToAdd != OVERALL)
        {
            score = -score; // decrease by default
        }

        int asbScore = abs(score);

        if (dartToAdd == SECTOR)
        {
            if (asbScore == 0) return;
            if (asbScore > 20 && asbScore != 25 && asbScore != 50) return;
        }
        else if (dartToAdd == DOUBLE || dartToAdd == TRIPLE)
        {
            if (asbScore == 0) return;
            if (asbScore > 20) return;
        }
...
static void ui_event_ScreenDarts(lv_event_t* e)
{
    ...
    {
        lv_label_set_text(ui_DLabelPly1, gui_getPlayerName(DARTS_PLAYER1));
        lv_label_set_text(ui_DLabelPly2, gui_getPlayerName(DARTS_PLAYER2));
        lv_label_set_text(ui_DLabelPly3, gui_getPlayerName(DARTS_PLAYER3));
...
static void ui_ScreenDarts_screen_init(void)
{
    ...
    lv_keyboard_set_map(ui_DKeyboard, LV_KEYBOARD_MODE_USER_1, score_kb_map_num, score_kb_ctrl_num_map);
    lv_keyboard_set_mode(ui_DKeyboard, LV_KEYBOARD_MODE_USER_1);
...
void gui_darts_create()
{
    ...
    lv_obj_remove_event_cb(ui_DKeyboard, lv_keyboard_def_event_cb);
	lv_obj_add_event_cb(ui_DKeyboard, ui_event_ScoreKeyboardButtons, LV_EVENT_ALL, NULL);
