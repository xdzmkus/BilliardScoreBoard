...
{
    lv_textarea_set_text(comp[UI_COMP_OPTIONSKEYBOARD_OPTIONTEXTAREA], ""); // clear textarea

    lv_keyboard_set_mode(comp[UI_COMP_OPTIONSKEYBOARD_KEYBOARDBUTTONS], LV_KEYBOARD_MODE_USER_2); // set ABC mode
...
static void ui_event_comp_OptionsKeyboard_OptionTextArea(lv_event_t* e)
{
    ...
    {
        ui_comp_cb_t cb = ui_comp_get_callback(comp_KeyboardPanel[UI_COMP_OPTIONSKEYBOARD_OPTIONSKEYBOARD]);
        
        if(cb != NULL)
        {
            cb(comp_KeyboardPanel[UI_COMP_OPTIONSKEYBOARD_OPTIONTEXTAREA]);
        }
...
lv_obj_t* ui_OptionsKeyboard_create(lv_obj_t* comp_parent)
{
    ...
    *callback = NULL;
    ...
    lv_obj_add_event_cb(cui_OptionsKeyboard, set_component_callback_event_cb, LV_EVENT_SET_COMP_CALLBACK, callback);
    lv_obj_add_event_cb(cui_OptionsKeyboard, get_component_callback_event_cb, LV_EVENT_GET_COMP_CALLBACK, callback);
    lv_obj_add_event_cb(cui_OptionsKeyboard, del_component_callback_event_cb, LV_EVENT_DELETE, callback);
